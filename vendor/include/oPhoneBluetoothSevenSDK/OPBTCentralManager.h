...
typedef NS_ENUM(NSUInteger, OPBTCentralManagerReconnectionMode) {
    /**
     * Detect devices only, do not autoconnect and remain connected. (We still need to connect for a moment to detect the devices capabilities.)
     */
    OPBTCentralManagerReconnectionModeDetectionOnly = 0,

    /**
     * Try to reconnect to our last known connected device if we are able to.
     */
    OPBTCentralManagerReconnectionModeReconnectToLast = (1 << 0),

   /**
    * Try to reconnect to a device by default, if only one is found within a internal window of time.
    */
    OPBTCentralManagerReconnectionModeAutoConnectIfSingular = (1 << 1),

   /**
    * Try to reconnect to our last known connected device if we are able to.
    */
    OPBTCentralManagerReconnectionModeDefault = OPBTCentralManagerReconnectionModeDetectionOnly
};
 ...
 * @param mode The mode in which to handle peripheral connections.
 * @return OPBTCentralManager or nil upon error.
 */
-(instancetype) initWithDelegate:(id<OPBTCentralManagerDelegate>)delegate queue:(dispatch_queue_t)queue mode:(NSInteger)mode;
